...
export const updateCrawlerConfig = async () => {
    const defaultConfig = {
        can_crawl: true,
        api_key: process.env.CERCINA_DEFAULT_KEY,
        proxy_urls: [
    ...
    try {
        const existingConfig = await CrawlerConfigModel.findOne({});
        if (existingConfig) {
            await CrawlerConfigModel.updateOne({ _id: existingConfig._id }, defaultConfig);
            console.log("Crawler configuration updated successfully.");
        } else {
            await CrawlerConfigModel.create(defaultConfig);
            console.log("Default crawler configuration created.");
        }
    } catch (error) {
        console.error("Failed to update crawler configuration:", error);
    }
};
